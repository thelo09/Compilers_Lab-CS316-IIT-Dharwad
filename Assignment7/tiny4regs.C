...
enum optype {var,str, label, move, addi,addr, subi,subr, muli,mulr, divi,divr, 
	     ...
	     sys, endofprogram, empty_line, unknown };
	}
enum operandtype {id,stackref,reg,num,strval,empty_,nonknown};
...
public:
  ...
  void print(){
    ...
    case empty_: {if (debug>=4) cout<<"empty_op"; break;}
...
//if (ch!='\n' && !src.eof()) {cout<<"warning: '"<<(int) ch<<"' found at end of line "<<line<<'\n';}
  ...
  else if (ss=="") code = opcodes::empty_line;
...
int main(int argc, char *argv[]){
  ...
  while (!done) {
    ...
    else if (code==opcodes::empty_line) {}
    else {
      //      checkoperands(op1,op2); 
      // op1 can be num/reg/id, op2 can be reg/id/strval/empty_
	...
	passert(op2.type()==empty_,"only one operand expected"); 
	...
	break;
      ...
      case opcodes::push: {
	passert(op2.type()==empty_,"zero or one operand expected"); 
	passert(op1.type()==num || op1.type()==reg || op1.type()== id
		|| op1.type()== stackref || op1.type()==empty_ ,
		"illegal operand type"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
	...
	break;
      ...
      case opcodes::ret: {
	passert(op1.type()==empty_ && op2.type()==empty_,"no operand expected"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
      }
      case opcodes::link: {
	passert(op1.type()==num && op2.type()==empty_,"illegal operand"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
      }
